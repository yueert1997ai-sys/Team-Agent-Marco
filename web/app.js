import { initializeStorage, getConversation, listConversations, loadAgentProfiles, loadPreferences, loadProviders, removeProviderSecret, saveAgentProfiles, saveConversation, savePreferences, saveProviderSecret, saveProviders, DEFAULT_AGENT_PROFILES } from "./storage.js";
import { PROVIDER_DEFAULTS, consultProvider, detectProvider, generatePrimary } from "./providers.js";
...
function fillSuggestion(text) { $("chatInput").value = text; resizeInput(); $("chatInput").focus(); }
...
async function sendMessage(event) {
  ...
  if (!primary) { showPage("settings"); toast("请先连接一个模型。DeepSeek 会优先成为总控。", true); return; }
  ...
  try {
    ...
    if (preferences.consultExperts) {
      ...
      if (experts.length) {
        ...
        const tasks = experts.map(async (provider) => {
          const step = addProcess(provider.id, `${agentName(provider)} 发言`, "等待接口返回…", "running");
          try {
            const result = await consultProvider(provider, conversation.messages, preferences.timeoutMs, getAgent(provider.id));
            updateProcess(step, result.text || "没有有效内容", "done");
            if (result?.text) {
              const note = `${agentName(provider)}（${getAgent(provider.id).role || provider.label}）：${result.text}`;
              internalNotes.push(note);
              consulted.push(agentName(provider));
            }
          } catch (error) { updateProcess(step, readError(error), "error"); }
        });
        await Promise.all(tasks);
      }
    }

    const primaryStep = addProcess(primary.id, `${agentName(primary)} 整合`, internalNotes.length ? "正在阅读其他 Agent 的发言…" : "无辅助 Agent，直接生成回答…", "running");
    ...
    setStatus("发送失败");
  } finally { sending = false; $("sendButton").disabled = false; $("chatInput").focus(); }
}
...
async function detectAndSaveKey() {
  ...
  try {
    ...
    renderProviders(); renderPrimaryState(); renderAgentProfiles(); toast(`${detected.label} 已连接`);
  } catch (error) { showDetectMessage(readError(error), true); }
  finally { button.disabled = false; button.textContent = "连接"; }
}

async function persistPreferences() {
  preferences = { ...preferences, consultExperts: $("consultExperts").checked, showProcess: $("showProcess").checked, maxOutputTokens: clampInteger($("maxOutputTokens").value, 100, 128000, 4000), timeoutMs: clampInteger($("timeoutMs").value, 5000, 600000, 120000) };
  await savePreferences(preferences);
...
async function persistAgents() {
  ...
  renderProviders(); renderPrimaryState(); renderAgentProfiles(); toast("Agent 配置已保存");
}

function renderSettings() { $("consultExperts").checked = preferences.consultExperts; $("showProcess").checked = preferences.showProcess; $("maxOutputTokens").value = preferences.maxOutputTokens; $("timeoutMs").value = preferences.timeoutMs; $("processPanel").classList.toggle("hidden", !preferences.showProcess); toggleCustomFields(); renderProviders(); renderPrimaryState(); }
...
function renderProviders() {
  ...
  box.querySelectorAll(".remove-provider").forEach((button) => button.onclick = async () => { const removedId = button.dataset.provider; await removeProviderSecret(removedId); providers = providers.filter((provider) => provider.id !== removedId); if (preferences.primaryProviderId === removedId) { preferences.primaryProviderId = providers.find((provider) => provider.id === "deepseek")?.id || providers[0]?.id || "deepseek"; await savePreferences(preferences); } await saveProviders(providers); renderProviders(); renderPrimaryState(); renderAgentProfiles(); toast("已移除模型"); });
}
...
function renderPrimaryState() { const primary = getPrimaryProvider(); const label = primary ? `${agentName(primary)} · ${primary.model}` : "未连接"; $("primaryModelBadge").textContent = primary ? agentName(primary) : "未连接"; $("connectionSummary").textContent = primary ? `总控：${label}` : "等待添加模型"; $("chatStatus").textContent = primary ? `${agentName(primary)} 将作为当前总控` : "请先在设置中添加模型"; const subtitle = $("welcomeSubtitle"); if (subtitle) subtitle.textContent = primary ? `${agentName(primary)} 负责最终回答，其他 Agent 会在右侧显示发言。` : "添加 DeepSeek、智谱 GLM 或其他模型后即可开始。"; }
...
function agentAvatar(id) { if (id === "input") return "你"; if (id === "error") return "!"; const agent = getAgent(id); return (agent.avatar || agent.displayName || id).slice(0, 4); }
...
async function getOrCreateConversation(firstText) { if (activeConversationId) { const existing = await getConversation(activeConversationId); if (existing) return existing; } const now = new Date().toISOString(); const conversation = { id: crypto.randomUUID(), title: firstText.replace(/\s+/g, " ").slice(0, 32), createdAt: now, updatedAt: now, messages: [] }; activeConversationId = conversation.id; return conversation; }
...
function addProcess(agentId, title, content, state = "pending") { if (!preferences?.showProcess) return null; const list = $("processList"); list.querySelector(".process-empty")?.remove(); const id = `process-${++processCounter}`; const node = document.createElement("article"); node.id = id; node.className = `process-item ${state}`; node.innerHTML = `<div class="process-title"><span class="process-name"><span class="process-avatar">${escapeHtml(agentAvatar(agentId))}</span><span>${escapeHtml(title)}</span></span><em>${stateLabel(state)}</em></div><pre>${escapeHtml(content)}</pre>`; list.append(node); list.scrollTop = list.scrollHeight; return id; }
function updateProcess(id, content, state = "done") { if (!id) return; const node = $(id); if (!node) return; node.className = `process-item ${state}`; node.querySelector("em").textContent = stateLabel(state); node.querySelector("pre").textContent = content; $("processList").scrollTop = $("processList").scrollHeight; }
