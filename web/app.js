...
function renderProviders() {
  ...
  box.innerHTML = providers.map((provider) => {
    ...
    return `<div class="provider-row"><div class="provider-title"><strong>${escapeHtml(agentName(provider))}${isPrimary ? '<span class="primary-tag">总控</span>' : ""}</strong><small>${escapeHtml(provider.label)} · ${escapeHtml(provider.baseUrl)}</small></div><input class="provider-model" data-provider="${escapeHtml(provider.id)}" value="${escapeHtml(provider.model)}" aria-label="模型名称">${isPrimary ? '<span></span>' : `<button class="set-primary" data-provider="${escapeHtml(provider.id)}">设为总控</button>`}<button class="danger remove-provider" data-provider="${escapeHtml(provider.id)}">移除</button></div>`;
  }).join("");
...
function renderAgentProfiles() {
  ...
  const ids = Array.from(new Set([...Object.keys(DEFAULT_AGENT_PROFILES), ...providers.map((provider) => provider.id)]));
  box.innerHTML = ids.map((id) => {
    const provider = providers.find((item) => item.id === id) || PROVIDER_DEFAULTS[id] || { label: id };
    const agent = getAgent(id, provider);
    return `<article class="agent-card" data-agent="${escapeHtml(id)}"><div class="agent-card-head"><strong>${escapeHtml(agent.displayName || provider.label)}</strong><small>${escapeHtml(provider.label || id)}</small></div><label>名字<input data-field="displayName" value="${escapeHtml(agent.displayName || "")}"></label><label>定位<input data-field="role" value="${escapeHtml(agent.role || "")}"></label><label>性格<textarea data-field="personality" rows="2">${escapeHtml(agent.personality || "")}</textarea></label><label>自定义提示词<textarea data-field="systemPrompt" rows="4">${escapeHtml(agent.systemPrompt || "")}</textarea></label></article>`;
  }).join("");
}
...
function renderPrimaryState() {
  ...
  if (subtitle) subtitle.textContent = primary ? `${agentName(primary)} 负责最终回答，其他 Agent 会在右侧显示发言。` : "添加 DeepSeek、智谱 GLM 或其他模型后即可开始。";
}
...
function getAgent(id, provider = null) { return agentProfiles[id] || defaultAgentFor(provider || providers.find((item) => item.id === id) || { id, label: id }); }
function agentName(provider) { return getAgent(provider.id, provider).displayName || provider.label; }
function defaultAgentFor(provider) { return DEFAULT_AGENT_PROFILES[provider.id] || { displayName: provider.label || provider.id, role: "自定义 Agent", personality: "直接、清晰、给可执行建议。", systemPrompt: `你是 ${provider.label || provider.id}，请作为 Team Agent Marco 的一个 Agent 发言。` }; }
